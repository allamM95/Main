...
# Shopify API credentials, API URL and Transaction URL from external cfg file

variables = readConfg.read_variables()

if len(variables) == 0:
    print("Configuration file is empty please contact developer. \n Program aborted...")
    time.sleep(2)
else:
    for variable in variables:
        ...
        elif variable.startswith('password'):
            password = variable_value
        elif variable.startswith('shop'):
            shop = variable_value
        elif variable.startswith('api_url'):
            api_url = f"" + variable_value
        elif variable.startswith('transaction_url'):
            var_transaction_url = f"" + variable_value
        ...
        for index, order_number in enumerate(order_numbers):
            ...
            if response.status_code == 200:
                ...
                if orders:
                    ...
                    #*********************** TO BE REMOVED ***********************
                    # commented out old way of reseting the transaction URL 
                    ...
                    transaction_url = var_transaction_url
                    transaction_data = {
                        "transaction": {
                            "kind": "capture"
                        }
